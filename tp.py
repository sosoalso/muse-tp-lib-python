...
def tp_set_button(tp, port, btn, state):
    ...
    """
    if tp:
        tp.port[port].channel[btn].value = state


# ---------------------------------------------------------------------------- #
def tp_set_button_in_range(tp, port, btn_start, btn_range, idx_cond):
    """
    tp_set_button_in_range 함수는 tp 객체의 모든 요소에 대해 지정된 port와 btn의 상태[켜짐|꺼짐] 를 설정합니다.
    """
    for i, btn in enumerate(range(btn_start, btn_start + btn_range + 1)):
        if tp:
            tp.port[port].channel[btn].value = idx_cond == i + 1
...
def tp_send_level(tp, port, lvl, value):
    ...
    """
    if tp:
        tp.port[port].level[lvl].value = value
